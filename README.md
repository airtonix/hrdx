...
Everything is clickable: workspace, tab, and pane rows in the sidebar, the main tab bar, menus, and the settings entry at the bottom. Git worktrees from the same repository are grouped together; use the workspace context menu's **Open worktree** action to add an existing worktree, or **Create worktree...** to create one.

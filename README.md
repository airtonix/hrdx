...
The default worktree command is `git worktree add {{base_worktree_path}}/.worktrees/{{name}} -b {{name}}`. Worktree commands support `{{name}}`, `{{path}}`, `{{base}}`, and `{{base_worktree_path}}`. The application quotes interpolated values.

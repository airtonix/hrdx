...
#   curl -fsSL https://raw.githubusercontent.com/patriceckhart/hrdx/main/install.sh | bash -s -- v0.0.1 ~/bin
#
...
#   HRDX_VERSION   same as $1
#   HRDX_PREFIX    same as $2
#
...
VERSION="${1:-${HRDX_VERSION:-latest}}"
PREFIX="${2:-${HRDX_PREFIX:-}}"
...
case "$uname_s" in
  Linux)   OS=linux ;;
  Darwin)  OS=darwin ;;
  *) die "unsupported os: $uname_s (hrdx needs a unix pty)" ;;
esac
...
msg "installed $("$PREFIX/$BINARY" --version 2>/dev/null || echo hrdx)"
msg "run:  hrdx          (interactive tui)"
msg "run:  hrdx --help   (all flags)"

...
const historyDefaultLimit = 5000

// pushHistory copies a line into the scrollback buffer. Alt-screen content
// (full screen apps) is not recorded, matching normal terminal behavior.
func (t *State) pushHistory(line []Glyph) {
	if t.mode&ModeAltScreen != 0 {
		return
	}
	if t.historyLimit == 0 {
		t.historyLimit = historyDefaultLimit
	}
...
// HistoryLen returns the number of scrollback lines available. Callers must
...
func (t *State) HistoryLine(index int) []Glyph {
...
func (t *State) scrollUp(orig, n int) {
	n = clamp(n, 0, t.bottom-orig+1)
	// Lines scrolling off the very top of the screen enter scrollback.
	if orig == 0 {
		for i := 0; i < n && i <= t.bottom; i++ {

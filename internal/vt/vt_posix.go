...
// Write parses input and writes terminal changes to state. PTY reads can
// split a UTF-8 rune across calls, so retain an incomplete suffix until the
// next write instead of dropping bytes from box-drawing characters.
